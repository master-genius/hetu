...
终端使用 WebGL 加速渲染文字。长时间运行颜色组合较多的 TUI 程序（如 AI 命令行工具）时，
极少数情况下可能出现文字乱码——字符内容本身正确，只是纹理缓存出现了不一致。

- **Ctrl+Shift+R**：重建当前终端的渲染缓存，无需退出 TUI 程序，对话记录不丢失。
- 若频繁出现，可执行 `reset` 命令完全重置终端（会清空屏幕，TUI 程序需重新打开）。
...
| Ctrl+Shift+R | 重建当前终端的渲染缓存（出现乱码时使用，无需退出 TUI 程序） |
