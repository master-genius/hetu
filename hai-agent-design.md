...
├── ai-config.json         ← AI Agent 配置（模型定义、执行策略、UI 偏好）
└── ai-keys.json           ← API 密钥（加密/权限隔离存储，不进入 git）
```
...
```json
{
  ...
  "models": {
    "deepseek-v3": {
      ...
      "endpoints": [
        { "url": "https://api.deepseek.com/v1", "key_id": "ds-key-1" },
        { "url": "https://api.deepseek.com/v1", "key_id": "ds-key-2" }
      ],
...
`ai-keys.json` 内容（密钥与配置分离，防误提交）：

```json
{
  "ds-key-1": "sk-xxx...",
  "ds-key-2": "sk-yyy...",
  "claude-key-1": "sk-ant-zzz..."
}
```

`endpoints[].key_id` 引用 `ai-keys.json` 中的密钥条目，两者通过 `key_id` 关联。
...
├── config.rs              ← ai-config.json + ai-keys.json 存取
...
| API 密钥隔离 | 存储于 `ai-keys.json`，与 `settings.json` 分离，不进入 git |
