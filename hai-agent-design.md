...
│  │  // 系统提示词每次请求前重建（含最新 Pane 列表），不进 history  │ │
│  │  loop {                                                          │ │
│  │    if aborted { break }                                          │ │
│  │    // 上下文窗口管理：超限时截断历史                             │ │
│  │    trim_history(&mut history, max_tokens)                        │ │
│  │    system_prompt = build_system_prompt(&tab_panes)               │ │
│  │    // chat_stream 通过 channel 推送事件，返回 Ok/Err              │ │
│  │    provider.chat_stream(&history, &tools, &system_prompt, &tx)   │ │
│  │      .await?;                                                     │ │
│  │    // 主循环消费 channel 事件                                     │ │
│  │    while let Some(event) = rx.recv().await {                       │ │
│  │      match event {                                                │ │
│  │        Message { done: true, .. } → break  // 文本结束            │ │
│  │        ToolStart { name, args, target_pane } =>                   │ │
│  │          if aborted { break }                                    │ │
│  │          match mode {                                             │ │
...
│  │        // AskUser 工具：LLM 通过 tool calling 向用户提问          │ │
│  │        // ReAct 循环对其做特殊处理（推送事件 + 暂停等待）         │ │
│  │      }                                                            │ │
│  │    }                                                              │ │
│  │  }                                                                │ │
│  └────────────────────────────────────────────────────────────────┘ │
...
│  │  run_command   ───►  target pane 的连接 → 本地 tokio::process  │ │
│  │                         / SSH exec channel（独立 channel，     │ │
│  │                         不干扰用户 PTY）                        │ │
...
│  │  · 密钥：存于 ai-config.json（app_data_dir，0600 权限），    │ │
│  │    与 settings.json 分离，不进 git                              │ │
│  └────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────┘
```
...
enum Tool {
    ...
    // 命令执行
    // 本地：tokio::process::Command（非交互，捕获 stdout/stderr/exit code）
    // 远程：SSH exec channel（独立 channel，不干扰用户 PTY）
    ...
    // 向用户提问（LLM 通过 tool calling 触发，非 LLM 原生响应类型）
    AskUser      { question: String, choices: Vec<UserChoice> },
...
target_pane → TabManager.getPane(tab_id, target_pane)
  ├── 本地 Pane → tokio::process::Command（run_command）
  │                / std::fs（read_file/write_file/list_dir）
  └── SSH Pane  → 通过 conn_id 走 SFTP / SSH exec channel
                   （复用已有连接，无需重建认证）
                   · SFTP 懒初始化（首次调用时建立）
                   · exec channel 独立于用户 PTY，不干扰终端
                   · exec channel 的环境变量可能与用户 shell 不同（PATH 等）
```
...
Rust: Tool::execute(ReadTerminal { target_pane, lines })
  → Channel<AgentEvent> 推送内部事件 ReadTerminalRequest
  → 前端监听 channel，读取 term.buffer.active
  → 前端 invoke("agent_terminal_data", { session_id, text })
...
- 使用 Tauri Channel（非 `app.emit()`），与 AgentEvent 流共用同一 channel
...
Agent 在 Rust 后端执行，需要各 Pane 的 cwd 来构建系统提示词和设置工作目录。但远程 Pane 的 cwd 仅在前端（OSC 7 更新 `pane.cwd`），后端不持有。

**方案 B（采用）**：不改 `pane.ts` 的 OSC 7 handler，由 Agent Modal 在 `show()` 时主动收集 Tab 内所有 Pane 的 cwd（前端已有 `pane.cwd`），通过 `agent_spawn` / `agent_update_panes` 命令一次性传给后端。

| Pane 类型 | cwd 来源 | 获取方式 |
|-----------|---------|----------|
| 本地 | `/proc/<pid>/cwd` 或前端 `pane.cwd` | `collectPanes()` 读取前端状态 |
| 远程 | OSC 7 → 前端 `pane.cwd` | `collectPanes()` 读取前端状态 |
...
pub struct PaneCtl {
    ...
    pub cwd: tokio::sync::Mutex<Option<String>>,  // 预留（方案 B 不依赖此字段）
}
```

**为什么选方案 B**：不改原有 `pane.ts` 的 OSC 7 handler 代码——AI 功能应完全隔离，不侵入原有终端逻辑。`pane_set_cwd` command 保留但当前不使用。

**兜底方案**：若前端 `pane.cwd` 为空（用户 shell 未触发 OSC 7），Agent 通过 SSH exec `pwd` 实时获取。

### 4.8 AskUser 工具机制

LLM 没有原生的"带选项提问"响应类型。`AskUser` 作为标准工具定义，LLM 通过 tool calling 触发提问。系统提示词中指示 Agent 在遇到方向性歧义时主动使用此工具。

ReAct 循环对 `AskUser` 工具调用做特殊处理：

```
ToolStart { name: "ask_user", args: { question, choices } }
  → 不执行实际操作，而是推送 UserQuestion 事件
  → 暂停 tokio task，等待前端 answer_question(session_id, choice)
  → 将用户选择作为 tool_result 追加到对话历史
  → continue ReAct 循环（Agent 基于用户选择继续推理）
```

这样所有 Provider 实装统一，无需各自处理 Question 类型。
...
#[async_trait]
trait LlmProvider {
    /// 流式对话请求。所有中间事件（文本块、工具调用、思考过程）
    /// 通过 channel 推送，函数返回 Ok 表示流正常结束，Err 表示错误。
    /// 系统提示词不进入 messages，作为独立参数传入（每次请求前重建）。
    ...
    /// 默认实现用粗估：ceil(bytes / 3.5)（中英混合文本经验值）。
    /// OpenAI 实装可选用 tiktoken-rs 精确计数。
    fn estimate_tokens(&self, messages: &[Message]) -> usize;
}

// 不再有 LlmResponse 枚举。
// 所有中间事件通过 Channel<AgentEvent> 推送：
//   Message { content, done }          — 文本流（done=true 表示文本结束）
//   Thinking { content }              — 思考过程（Claude thinking / o1）
//   ToolStart / ToolOutput / ToolEnd  — 工具调用生命周期
//   Done                               — 整个响应结束
//   Error { message }                  — 错误
// chat_stream 返回 Ok(()) 仅表示流正常结束，具体事件已在 channel 中。
```
...
- **新增依赖**（Cargo.toml）：
  ...
  ```
  使用 `rustls-tls`（非 OpenSSL），与项目现有 russh/rustls 技术栈一致。
- **token 估算**：默认用 `ceil(bytes / 3.5)` 粗估（中英混合经验值），OpenAI 实装可选 `tiktoken-rs` 精确计数
...
Agent 事件流使用 **Tauri `Channel<AgentEvent>`**（Tauri 2.x 专为流式推送设计，类型安全，无事件名冲突）。现有 `app.emit()` 事件（pane-output/conn-state 等）不受影响，两套机制共存。

前端创建 Channel 并传给 `agent_spawn`：

```typescript
const channel = new Channel<AgentEvent>();
channel.onmessage = (event) => { /* 渲染事件 */ };
await invoke("agent_spawn", { tabId, mode, role, initialMessage, channel });
```
...
enum AgentEvent {
    ...
    // Agent 向用户提问（由 AskUser 工具触发，非 LLM 原生响应类型）
...
与 Ask 模式不同——Ask 是审批工具调用，提问是 Agent 在推理过程中遇到**需要用户决策的岔路口**。通过 `AskUser` 工具实现（LLM 通过 tool calling 触发，非 LLM 原生响应类型）。ReAct 循环中的处理：

```
ReAct loop:
  // chat_stream 通过 channel 推送事件
  provider.chat_stream(&history, &tools, &system_prompt, &tx).await?;
  // 事件处理
  ToolStart { name: "ask_user", args: { question, choices } }
    → channel.push(UserQuestion { question, choices })
    → 暂停 tokio task
    → 等待前端 answer_question(session_id, choice)
    → 将用户选择作为 tool_result 追加到对话历史
    → continue ReAct 循环（Agent 基于用户选择继续推理）
```
...
Agent 可以在系统提示词中被指示：遇到方向性歧义时使用 `ask_user` 工具向用户提问，附带选项和说明。
...
| Alt+H / 隐藏按钮 | 隐藏 Modal（不销毁会话，再次 `hai` 恢复） |
| ESC | 中止当前对话（不隐藏 Modal） |
...
providers
  ...
  │   ├── default_model             ← 该 provider 下的默认模型分组 ID
  │   └── models
  │       ├── deepseek-v3           ← 模型分组 ID
  │       │   ├── show_name         ← UI 显示名（可选，默认=分组 ID）
  │       │   └── endpoints[]       ← endpoint 列表（加权轮转）
  │       │       ├── [0] { id, url, key, weight, options, headers, body }
  │       │       └── [1] { id, url, key, weight, options }
  │       └── glm-5.2
  │           └── endpoints[]
  │               └── [0] { id: "Zhipu/GLM-5.2", url, key, weight: 5 }
  └── anthropic
      └── ...

default_provider                    ← 全局默认 provider（openai/anthropic）

roles
  ├── general       → { model }     ← 不指定则用 provider 的 default_model
  ├── code          → { model }
  └── debug         → { model }
```
...
```json
{
          ...
          "endpoints": [
            {
              ...
              "headers": { "HTTP-Referer": "https://hetushell.app" },
              "body": { "response_format": { "type": "json_object" } }
            }
          ]
        },
        "deepseek-v3": {
          "show_name": "DeepSeek V3",
          "endpoints": [
            { "url": "https://api.deepseek.com/v1", "key": "sk-yyy" }
          ]
        }
  ...
  "default_provider": "openai",
  "roles": {
    "code": { "model": "deepseek-v3" },
    "debug": { "model": "deepseek-v3" }
  }
}
```

**简化格式兼容：** endpoints 直接为数组时，`show_name` 默认 = 分组 ID；endpoint 无 `id` 时默认 = 分组 ID；无 `weight` 时默认 = 1。

**字段说明：**

| 字段 | 级别 | 说明 |
|------|------|------|
| `endpoints[].id` | endpoint | 实际发给 API 的 model 参数（同一分组内不同厂商可能不同） |
| `endpoints[].url` | endpoint | API 地址 |
| `endpoints[].key` | endpoint | API Key |
| `endpoints[].weight` | endpoint | 加权轮转：weight=N 连续用 N 次，weight=0 跳过（默认 1） |
| `endpoints[].options` | endpoint | OpenAI 标准 body 参数（max_tokens/temperature/top_p 等） |
| `endpoints[].headers` | endpoint | 自定义 HTTP 头（如 OpenRouter 的 HTTP-Referer/X-Title） |
| `endpoints[].body` | endpoint | 非标准 body 参数透传（response_format/logit_bias/n 等，直接合并进 request body） |
| `default_model` | provider | 该 provider 下的默认模型分组 ID |
| `default_provider` | 顶层 | 全局默认 provider 类型 |
| `roles.<role>.model` | role | 角色绑定的模型分组 ID（不指定则用 provider 的 default_model） |

**设计要点：**

| 规则 | 说明 |
|------|------|
| 同分组多 endpoint | 加权轮转池，weight=N 连续用 N 次后轮转 |
| 429 限流 | `force_rotate` 清零配额，立即切下一个 endpoint |
| 自定义 headers | 支持平台特定的 HTTP 头（OpenRouter/Together/Azure 等） |
| extra body | 非标准参数直接合并进 request JSON，全面支持 OpenAI 规范 |
| 密钥安全 | 文件在 app_data_dir 下，权限 0600，不进入 git |

### 角色绑定模型

每个角色可独立指定 `model`（分组 ID），不填则用 `default_provider` 下的 `default_model`。设置面板中给出建议：
...
src-tauri/src/agent/
├── mod.rs                 ← AgentManager + Tauri commands（agent_spawn 等）
├── loop.rs                ← ReAct 循环（per-session tokio task）
├── tools.rs               ← Tool trait + 工具注册表 + 本地实现
├── tools_ssh.rs           ← SSH 远程工具实现（规划，后续实现）
├── tools_terminal.rs      ← read_terminal 前后端往返（oneshot + 超时）
├── mcp.rs                 ← MCP Server 管理（子进程 + stdio + tools/list|call）
├── provider.rs            ← LlmProvider trait + OpenAiProvider 实装
├── provider_anthropic.rs  ← Anthropic API 适配（Phase 4）
├── session.rs             ← Session 状态机
├── config.rs              ← ai-config.json 存取 + 角色模型绑定
└── types.rs               ← 共享类型（AgentEvent, ToolResult, Message 等）

src-tauri/src/
└── local.rs               ← +hai shell 脚本
└── lib.rs                 ← +invoke_handler 追加 agent commands
                             +PaneCtl.cwd 字段

src/
└── main.ts                ← +Tab 生命周期管理（挂载/卸载 AgentSession）
└── pane.ts                ← +OSC 1733 handler
                             +前端 collectPanes 收集 cwd 传给后端
└── ipc.ts                 ← +agent commands 封装
```
...
渲染栈：
  ...
  marked-katex-extension    ← marked 扩展，注册 $...$ / $$...$$ tokenizer → KaTeX 渲染
  highlight.js              ← 代码语法高亮（按语言自动检测）
  diff2html + diff          ← Diff 统一视图（GitHub 风格）
  ansi-to-html              ← ANSI 转义序列 → 彩色 HTML
  mermaid                   ← Mermaid 图表客户端渲染（可选，按需加载）
  katex                     ← 数学公式渲染引擎（被 marked-katex-extension 调用）
```

均为纯前端库，无新增 Tauri/系统依赖。全部 tree-shakable，不加载即不打包。

**加载策略（静态 vs 动态 import）：**

| 库 | 加载方式 | 理由 |
|----|----------|------|
| `marked` | 静态 import | 核心渲染，体积小（~30KB） |
| `highlight.js` | 静态 import（仅常用语言） | 按需注册语言：rust, python, js/ts, go, java, c/cpp, shell, yaml, json, toml, sql。全量包含所有语言极大（~1MB+），禁止全量导入 |
| `diff2html` + `diff` | 静态 import | 体积适中（~50KB），Agent 常用 diff 展示 |
| `ansi-to-html` | 静态 import | 体积小（~10KB），工具输出渲染必需 |
| `mermaid` | 动态 import（`await import('mermaid')`） | 压缩后 ~500KB+，仅当内容含 mermaid 代码块时加载 |
| `katex` + `marked-katex-extension` | 动态 import | ~200KB+，仅当内容含 `$...$` 或 `$$...$$` 时加载 |

**KaTeX 集成方式：**

`marked` 核心不支持数学公式，`$...$` 会被当作普通文本。通过 `marked-katex-extension` 扩展接入：
...
marked.use(markedKatex({
  throwOnError: false,  // 公式解析失败时保留原文，不抛异常
  output: "html",       // 输出 HTML（非 mathml）
}));
```

扩展注册后，`$E=mc^2$`（行内）和 `$$\int_0^1 f(x)dx$$`（块级）会被 KaTeX 渲染为 HTML。扩展内部处理了代码块内 `$` 转义、行内/块级区分、AMS 环境等边界 case。

延迟加载策略——首次检测到公式语法时动态 import 并注册扩展：
...
async function ensureKatex() {
  ...
  const [{ default: katex }, { default: markedKatex }] = await Promise.all([
    import("katex"),
...
push(text: string) {
  this.buffer += text;
  if (!katexReady && /\$[$]?/.test(text)) {
    ensureKatex().then(() => this.rerender());
    return;
  }
  this.rerender();
}
...
未加载 KaTeX 时，`$...$` 由 marked 原样输出为文本，不影响正常 Markdown 渲染。

### 14.3 流式渲染——冻结块方案

Agent 的 `Message` 事件是流式到达的，不能等全部到达再渲染（SSE 的 chunk 粒度可能是几个 token）。

**核心思路：** 每次 chunk 对完整 buffer 执行 `marked.parse()` → HTML，按 top-level 块级元素（`<p>`, `<pre>`, `<div>`, `<ul>` 等）分割，除最后一块外全部**冻结**（clone 到正式 DOM，不再触碰），只有最后一块（活跃块）每次 chunk 更新。

**原理：** Markdown 语法保证——一旦 `\n\n` 出现，其前的块在语法上已完成，后续 chunk 不可能改变它的类型。`marked` 输出的 top-level children 中，除最后一个外都是稳定的。

**为什么不需要预处理：** `marked` 自带容错——未闭合 ```` ``` ```` 渲染为代码块到末尾，未闭合 `**` 渲染为字面量。活跃块每次 chunk 重新解析，临时错误自我修正。

**为什么不用 BlockParser 状态机：** 手动 block 分割无法处理嵌套（列表中的代码块、引用中的表格），正确性应靠 `marked` 保证，不自己写 Markdown 解析器。
...
buffer 累积
  │
  ▼
rAF 节流（≤60fps，合并多 chunk）
  │
  ▼
marked.parse(buffer) → HTML（全量解析，20KB < 0.3ms）
  │
  ▼
temp container → top-level children[]
  │
  ├── children[0 .. n-2] = 已完成块 → 冻结（clone 到正式 DOM）
  │     · 代码块在冻结时一次性高亮（此时块已完整）
  │     · 冻结后不再触碰：选区/复制/事件监听器全部保留
  │
  └── children[n-1] = 活跃块 → 更新 activeEl.innerHTML
        · 流式中的活跃块不高亮（仍在变化）
        · 临时语法错误在下次 chunk 自然修正
  │
  ▼
流结束（done: true）→ 冻结最后一块 + 最终高亮
```
...
class StreamingMarkdown {
  ...
  private activeEl: HTMLElement;      // 最后一块的 DOM 容器
  private frozenCount = 0;            // 已冻结的块数
  private buffer = "";
  private pending = false;            // rAF 节流标志
  ...
  private rerender() {
    const html = marked.parse(this.buffer, { async: false });
    ...
    // 防御：若块数减少（marked 版本行为变化等），移除多余冻结块
    while (this.frozenCount > newFrozen) {
  ...
  private highlightIfCode(el: HTMLElement) {
    el.querySelectorAll('pre code:not([data-hl])').forEach((code) => {
      hljs.highlightElement(code as HTMLElement);
      (code as HTMLElement).dataset.hl = 'done';
    });
  }
}
```
...
| 特性 | 效果 |
|------|------|
| 冻结块不被触碰 | 选区/复制/事件监听器全部保留 |
| 高亮只在冻结时执行 | 流式中代码块不高亮（仍在变），闭合时一次性高亮 |
| 无 DOM diff | 直接按 children 索引操作，不做 outerHTML 比较 |
| marked.parse() 全量 | 正确性靠 marked 保证，不自己写解析器 |
| rAF 节流 | 多 chunk 同帧到达只渲染一次 |

**性能：**

| 响应大小 | marked.parse() | DOM 操作 | 高亮 | 合计/帧 |
|----------|---------------|---------|------|---------|
| 10KB | ~0.3ms | ~0.05ms | ~2ms/块（仅冻结时） | < 1ms |
| 50KB | ~1.5ms | ~0.05ms | 同左 | < 2ms |

60fps 绰绰有余。超长输出（200KB+）可优化为只解析 `buffer[frozenOffset..]`，后续按需。
...
- [ ] Rust：添加 `reqwest`（HTTP/2 + SSE + rustls，`default-features = false`）
- [ ] `LlmProvider` trait 定义 + `OpenAiProvider` 实装
- [ ] `ai-config.json` 读写（0600 权限）
- [ ] 前端渲染基础：引入 `marked` + `highlight.js`（静态 import，仅常用语言）
- [ ] Agent Modal 基础 DOM 结构（复用 himage 容器 + 消息区 + 输入框）
- [ ] Tauri `Channel<AgentEvent>` 前后端联通
...
- [ ] Ask 模式（工具确认）
- [ ] Plan 模式（先计划后执行）
- [ ] Agent 提问机制（AskUser 工具 + answer_question）
- [ ] read_terminal 工具（读 xterm buffer，前后端往返）

### Phase 3 — SSH 远程

- [x] 工具通过 SSH conn_id 路由到远程
- [x] SFTP 文件操作（懒初始化 session）
- [x] SSH exec channel 命令执行（独立 channel，不干扰 PTY）
- [x] 远程 Pane 的 cwd 同步（方案 B：前端 collectPanes 主动收集）
- [x] 远程路径解析（SFTP `canonicalize`）
...
| 检查项 | 实现 |
|--------|------|
| 文件操作路径限制 | `read_file` / `write_file` / `list_dir` 做 path canonicalization，限制在 target pane 的 cwd 子树内（可配置解除）。本地用 `std::fs::canonicalize`；远程用 SFTP `realpath` 解析后检查前缀。注意：`run_command` 天然可绕过路径限制（如 `cat /etc/passwd`），这是命令执行灵活性的固有代价，不做硬阻断 |
| 危险命令检测 | `run_command` 对 `dangerous_commands` 列表做关键字检测（`rm`、`dd`、`mkfs`、`shutdown` 等）。命中时在 Auto 模式下自动升级为 Ask 确认；Ask 模式下本就需确认。注意：子串匹配不是安全边界，是提示性约束——用户和 Agent 可通过变体绕过，Ask 模式是真正的安全层 |
| HSSH_TOKEN 校验 | OSC 1733 载荷中的 token 必须与 pane 注入的一致 |
| API 密钥隔离 | 密钥在 `ai-config.json` 中（0600 权限），与 `settings.json` 分离，不进入 git |
...
**Channel 生命周期：** `Channel<AgentEvent>` 在 `agent_spawn` 时由前端创建并传入。Rust session task 持有它直到销毁。Alt+H / 隐藏按钮隐藏 Modal 不销毁 session——Channel 的 onmessage 仍有效，Modal 只是 hidden。再次 `hai` 恢复时复用已有 session，不新建 Channel。ESC 不隐藏 Modal，而是中止当前 LLM 请求。点击空白区域不隐藏 Modal（AI 面板是工作面板，不同于图片查看器等消费型弹窗）。
...
Alt+H / 隐藏按钮 → Modal hide（display:none），session 保留
ESC → 中止当前 LLM 请求（不隐藏 Modal）
点击空白 → 无操作（AI 面板是工作面板，不因点击空白而消失）
...
- ESC 中止 + Alt+H 隐藏 + capture-phase keydown 模式（xterm 截获 bubble phase）
