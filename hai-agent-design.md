...
| Pane 类型 | cwd 来源 | 同步方式 |
|-----------|---------|----------|
| 本地 | `/proc/<pid>/cwd`（`local.rs::cwd(pid)` 已实现） | 后端直接读取，无需同步 |
| 远程 | OSC 7 → 前端 `pane.cwd` | 前端收到 OSC 7 后 invoke 同步给后端 |
...
pub struct PaneCtl {
    ...
    pub cwd: Mutex<Option<String>>,  // 远程 pane 的 cwd（前端同步）
}
```

**前端 OSC 7 handler 增加后端同步：**
...
this.term.parser.registerOscHandler(7, (data) => {
  try {
    ...
    if (url.protocol === "file:") {
      ...
      if (!this.isLocal) {
        void api.paneSetCwd(this.id, this.cwd).catch(() => {});
      }
    }
  } catch { /* 忽略 */ }
  return true;
...
新增 Tauri command `pane_set_cwd(pane_id, cwd)`，更新 `AppState.panes` 中对应 `PaneCtl.cwd`。

**兜底方案**：若后端 `cwd` 为 `None`（用户 shell 未触发 OSC 7），Agent 通过 SSH exec `pwd` 实时获取。
...
- [ ] SFTP 文件操作（懒初始化 session）
- [ ] SSH exec channel 命令执行（独立 channel，不干扰 PTY）
- [ ] 远程 Pane 的 cwd 同步（OSC 7 → `pane_set_cwd` command）
- [ ] 远程路径解析（SFTP `realpath`）
